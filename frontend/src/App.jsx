...
function App() {
  ...
  const [previewDoc, setPreviewDoc] = useState(null); // The document that is being previewed
  ...
  const handleChat = async (e) => {
    ...
    try {
      const response = await sendChatMessage(messageToSend, currentThreadId, modelProvider);
      const aiMsg = {
        role: 'assistant',
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <nav className="flex justify-between items-center bg-white/50 backdrop-blur-md p-4 rounded-3xl border border-white/20 shadow-soft">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-800">影音轉文字助手</span>
          </div>
          <div className="flex bg-slate-100 p-1.5 rounded-2xl">
            ...
            <button
              ...
              className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 ${view === 'library' ? 'bg-white shadow-sm text-primary' : 'text-text-muted hover:text-text'}`}
            >
              <Library size={16} />
              資料庫
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 ${view === 'chat' ? 'bg-white shadow-sm text-primary' : 'text-text-muted hover:text-text'}`}
            >
              <Sparkles size={16} />
              AI 問答
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 ${view === 'documents' ? 'bg-white shadow-sm text-primary' : 'text-text-muted hover:text-text'}`}
            >
              <FileBox size={16} />
              文件庫
            </button>
                ...
                <div className="md:col-span-3 flex items-center justify-between p-6 bg-primary/5 border border-primary/10 rounded-3xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-text-muted">轉錄結果已整理完畢。</p>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    ...
                    {saveStatus === 'saved' ? (
                      ...
                      >
                        {saveStatus === 'saving' ? <Loader2 className="animate-spin" size={18} /> : <BookMarked size={18} />}
                        加入我的資料庫
                      </button>
                    )}
                  </div>
            ...
            <section className="max-w-2xl mx-auto">
              <form onSubmit={handleLongSubmit} className="relative group">
                ...
                >
                  ...
                  <span>{isLoading ? '深層處理中...' : '開始轉譯'}</span>
                </button>
              </form>
              ...
              <div className="grid grid-cols-1 gap-8 animate-in fade-in zoom-in duration-500">
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    className={`btn-primary !px-12 !py-4 rounded-2xl flex items-center gap-3 shadow-xl ${saveStatus === 'saved' ? 'bg-green-500 hover:bg-green-500' : ''}`}
                  >
                    {saveStatus === 'saving' ? <Loader2 className="animate-spin" /> : <BookMarked />}
                    <span className="text-lg">{saveStatus === 'saved' ? '已儲存至資料庫' : '儲存轉譯結果'}</span>
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-text-muted mt-1">上傳 PDF 或 Word 文件，擴充 AI 的知識範圍。</p>
              </div>
                ...
                <div className="w-full h-full bg-white p-12 overflow-y-auto">
                  <div className="max-w-3xl mx-auto">
                    <div className="p-8 bg-slate-50 rounded-3xl border border-slate-100">
                      <p className="text-xs text-slate-400 font-bold uppercase mb-8 border-b border-slate-200 pb-4">提取出的文字內容 (Word 預覽)</p>
                      <div className="prose prose-slate max-w-none">
                        ...
                        <div className="mt-8 text-slate-800 text-lg leading-loose">
                           /* Word content preview logic would go here if we fetched textContent.
                          For now, we'll suggest downloading the file for full formatting. */
